...
export default function AdminDeposits({ pendingDeposits, handleConfirmDeposit, handleRejectDeposit, loading }) {
    if (loading) return <div className="adm-loading"><span>Memuat data deposit...</span></div>;
    if (pendingDeposits.length === 0) return <div className="adm-card"><div className="adm-empty">Tidak ada permohonan deposit baru yang pending.</div></div>;
                ...
                {pendingDeposits.map(dep => (
                    <div key={dep.id} className="adm-deposit-item">
                        <div>
                            ...
                            {dep.paymentProof && (
                                <a href={`${import.meta.env.VITE_API_BASE_URL}${dep.paymentProof}`} target="_blank" className="adm-deposit-proof" rel="noreferrer">
                                    <ExternalLink size={12} /> Lihat Bukti Transfer
                                </a>
                            )}
                        </div>
                        <div className="adm-deposit-actions">
                            <button onClick={() => handleRejectDeposit(dep.id)} className="btn btn-secondary" style={{ padding: '7px 12px', fontSize: '12px' }}>
                                <X size={14} /> Tolak
                            </button>
                            <button onClick={() => handleConfirmDeposit(dep.id)} className="btn btn-primary" style={{ padding: '7px 16px', fontSize: '12px' }}>
