...
// Record usage + deduct credits + emit SSE events
// Dedup by requestId (database-level) to prevent double-counting with webhook
async function recordUsage({ apiKey, requestId, endpoint, body, responseStatus, latencyMs, inputTokens, outputTokens, modelId }) {
  try {
    ...
    const actualModel = body?.model || modelId;
...
router.post('/chat/completions', proxyLimiter, async (req, res) => {
  ...
  try {
    ...
    if (ct.includes('text/event-stream')) {
      // Streaming: pipe SSE to client, extract usage from final chunk
          ...
          if (l.startsWith('data: ') && l.includes('choices')) {
            try {
              ...
              if (chunk.choices?.[0]?.finish_reason && !usageRecorded) {
                // Try to get real token counts from 9router response
                const inputTokens = chunk.usage?.prompt_tokens || chunk.usage?.input_tokens || 0;
                ...
                if (inputTokens > 0 || outputTokens > 0) {
                  await recordUsage({
                    ...
                    inputTokens, outputTokens, modelId: req.body.model || 'unknown',
                  });
                  usageRecorded = true;
                }
              }
      ...
      // Fallback: if stream ended without token data, record with estimation
      if (!usageRecorded) {
      ...
      // Non-streaming: record usage from response
              ...
              if (chunk.choices?.[0]?.finish_reason && !usageRecorded) {
                ...
                if (inputTokens > 0 || outputTokens > 0) {
                  await recordUsage({
                    ...
                    body: req.body, responseStatus: response.status,
                    latencyMs: Date.now() - startTime,
                    inputTokens, outputTokens, modelId: model || 'unknown',
                  });
                  usageRecorded = true;
                }
              }
