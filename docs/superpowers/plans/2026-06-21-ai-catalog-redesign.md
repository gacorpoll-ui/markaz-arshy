# AI Router Catalog Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Redesign the AI Router Catalog page with a modern, OpenRouter-inspired hybrid table-cards layout across all sections.

**Architecture:** Single file rewrite of `frontend/src/pages/AIRouterCatalog.jsx`. All 6 sections (hero, features, model catalog, pricing tiers, 3-step guide, footer CTA) are redesigned within this one component. No new files needed — the existing design system CSS variables handle theming.

**Tech Stack:** React 19, Lucide React icons, inline styles with CSS variables (`--text-primary`, `--color-primary`, `--border-color`, `--glass-bg`, etc.)

## Global Constraints

- Dark neon glassmorphism theme — use CSS variables from `index.css` (`--bg-surface`, `--border-color`, `--color-primary: #00f2fe`, `--color-secondary: #4facfe`, `--text-primary`, `--text-secondary`, `--text-muted`, `--glass-bg`)
- Fonts: `var(--font-title)` for headings, `var(--font-primary)` for body
- Icons from `lucide-react` only
- Pricing: database stores Rp per 1K tokens. `formatPrice()` shows per 1K, `formatPricePerM()` shows per 1M
- No external dependencies added
- Responsive: collapse grids on mobile, horizontal scroll for table
...
| File | Action | Purpose |
|------|--------|---------|
| `frontend/src/pages/AIRouterCatalog.jsx` | Modify | Full page redesign — all 6 sections |

---

### Task 1: Rewrite AIRouterCatalog.jsx — Complete Redesign

**Files:**
- Modify: `frontend/src/pages/AIRouterCatalog.jsx`

**Interfaces:**
- Consumes: API response from `/api/ai-router/providers` (array of provider objects with `models` array)
- Props: `{ user, token }` — `user` is null when not logged in, `token` is JWT string
- Produces: Full page render with 6 sections

- [ ] **Step 1: Replace the entire file content with the redesigned page**

Replace the complete content of `frontend/src/pages/AIRouterCatalog.jsx` with the following:
...
Open `http://localhost:5173/catalog/ai-router` and verify:
- Hero section renders with gradient title and code block
- Features strip shows 4 cards in a row
- Model catalog table shows all models with provider dots, badges, prices in Rp
- Clicking a row expands the detail panel
- "Use" button copies model ID
- Pricing tiers display correctly
- 3-step guide shows with connecting line
- Footer CTA renders

- [ ] **Step 3: Test responsive behavior**

Resize browser to mobile width (< 768px) and verify:
- Features strip collapses to 2 columns
...
- Click provider filter pills → table filters correctly
- Click "Use" button → copies model ID, shows "✓" feedback
- Click "Copy" on quick start → copies curl command
- Click "Copy Example" in expanded detail → copies model-specific curl
- Expand/collapse rows → chevron toggles, detail panel opens/closes

- [ ] **Step 5: Commit**

```bash
git add frontend/src/pages/AIRouterCatalog.jsx
git commit -m "redesign: AI Router catalog — hybrid table-cards layout with provider dots, action buttons, timeline steps"
```
